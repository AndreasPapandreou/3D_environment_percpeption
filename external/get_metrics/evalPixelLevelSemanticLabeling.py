...
def evaluateImgLists(predictionImgList, groundTruthImgList, town, type):
    ...
    for i in range(len(predictionImgList)):
        predictionImgFileName = "/media/andreas/lab/all_thesis_data/" + town + "/evaluation/predicted_processed_images/" + predictionImgList[i]
        groundTruthImgFileName = "/media/andreas/lab/all_thesis_data/" + town + "/evaluation/gt_processed_images/" + groundTruthImgList[i]
        
        if (path.exists(predictionImgFileName) and path.exists(groundTruthImgFileName)):
            pass
        else:
            continue
...
def convert_to_row(image):
    ...
    raw_image = np.zeros((img_height, img_width))
...
def evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix, instanceStats, perImageStats, args):
    ...
    try:
        ...
        predictionNp = convert_to_row(predictionNp)

        # predictionNp = predictionNp[:, :, 0]
        # print ("predictionNp shape = ", predictionNp.shape)
    except:
    ...
    try:
        ...
        groundTruthNp = convert_to_row(groundTruthNp)
...
def main():
    ...
    # evaluate
    evaluateImgLists(predictionImgList, groundTruthImgList, args, pathToFile)
